...
class CIsoCamera(object):
	...
	def image_to_world(self, u, v, disparity):
		...
		xWorld =   xCam * math.cos(self.extrinsic.pitch) + zCam * math.sin(self.extrinsic.pitch) + self.extrinsic.x
		zWorld = - xCam * math.sin(self.extrinsic.pitch) + zCam * math.cos(self.extrinsic.pitch) + self.extrinsic.z
...
def main(argv):
	...
	# Example of how to use this
        cam = CIsoCamera()
	cam.loadFromJson("[ROOT]/camera/train/tsinghuaDaimlerDataset/tsinghuaDaimlerDataset_2015-03-24_041424_000028651_camera.json")
	cam_vector = cam.image_to_camera(500,500,30)
	print "x: "+str(cam_vector[0])+ "  y: "+str(cam_vector[1])+"  z: "+str(cam_vector[2])
	world_vector = cam.image_to_world(500,500,30)
	print "x: "+str(world_vector[0])+ "  y: "+str(world_vector[1])+"  z: "+str(world_vector[2])

	# Example of how to use depth loading
	dep = depth.CDepth()
	dep.setCamera(cam)
	dep.readFromDisparityImage("[ROOT]/disparity/train/tsinghuaDaimlerDataset/tsinghuaDaimlerDataset_2015-03-24_041424_000028651_disparity.png")
